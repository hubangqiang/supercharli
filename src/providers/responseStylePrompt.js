function buildResponseStylePrompt(context = {}) {
  ...
  const lines = [
    ...
    "- Prioritize the user's concrete pressure point and keep reasoning actionable.",
    "- Avoid slogan-heavy output and repetitive fixed openings.",
    ...
    "- Vary sentence rhythm and structure across turns; avoid rigid scaffolding.",
    ...
    "- Ending discipline: optionally close with decision, risk, or one clear action line only when it helps.",
    "",
    `Detected user state: ${detected.id}.`,
    `Tone intensity: ${toneLevel} (repeatCount=${repeatCount}); style intent: ${detected.intent}.`,
    ...
    "When the same problem repeats, escalate precision and tighten checkpoint clarity.",
    "If user is emotional or offensive, stay composed and redirect to concrete problem-solving.",
    "Question usage rule: ask at most one concise clarification question only when critical information is missing.",
  ...
  if (context.focusMode) {
    lines.push(
      ...
      "- Provide only what is necessary now: objective, key checks, and next executable step.",
      "- Keep language concise and task-oriented; avoid motivational filler.",
    );
  }
...
function buildResponseStylePromptShort(context = {}) {
  ...
  return [
    ...
    "- Use concise professional sentences and adapt structure to user intent; no fixed format mandate.",
